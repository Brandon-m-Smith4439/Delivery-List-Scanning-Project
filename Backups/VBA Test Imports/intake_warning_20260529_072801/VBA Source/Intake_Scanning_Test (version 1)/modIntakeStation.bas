...
'   prevents an operator from accidentally scanning against yesterdayâ€™s or the
'   wrong stageâ€™s snapshot.
'
' Workflow note:
'   Public entry point: keep this name/signature stable unless every button,
'   form, timer, workbook event, and cross-module call is updated too.
...
Private Function PrepareCurrentStageBeforeSettingsChange(ByRef errorMessage As String) As Boolean
    ...
    If Not WaitForActiveQueueRequestsToFinishForSettingsChange(90, errorMessage) Then
        Exit Function
    End If
...
Public Sub SubmitQueuedScanFromBox()
    ...
    If Not IsValidBarcode(barcodeText) Then
        UpdatePanelForLocalError "BAD_FORMAT", "Barcode must be T200 + 12 digits."
        ClearStageScanBoxSilently
        Exit Sub
    End If
...
Public Function IsValidBarcode(ByVal barcodeText As String) As Boolean
    barcodeText = CleanBarcodeText(barcodeText)
    IsValidBarcode = (Len(barcodeText) = 16 And barcodeText Like "T200############")
End Function
...
Public Function DecodeBarcodeOrder(ByVal barcodeText As String) As Long
    barcodeText = CleanBarcodeText(barcodeText)
    If IsValidBarcode(barcodeText) Then
        DecodeBarcodeOrder = CLng(Mid$(barcodeText, 5, 6))
    End If
End Function
...
Public Function DecodeBarcodeItem(ByVal barcodeText As String) As Long
    barcodeText = CleanBarcodeText(barcodeText)
    If IsValidBarcode(barcodeText) Then
        DecodeBarcodeItem = CLng(Mid$(barcodeText, 11, 3))
    End If
End Function
...
Private Sub ApplyStageOutOfDateMarker(ByVal messageText As String)
    ...
    If rng.MergeCells Then rng.MergeArea.UnMerge
    rng.Merge

    With rng
        .Value = "SNAPSHOT OUT OF DATE - Keep scanning if needed. Click Refresh Snapshot ASAP."
...
'
' Why it exists:
'   Done/Error status is the masterâ€™s final answer; the intake workbook must
