...
def test_single_javascript_bundle_is_loaded() -> None:
    ...
    assert scripts == ["static/js/app.js?v=20260831-v0.471"]
...
def test_scan_time_pill_qty_headers_and_table_are_width_safe() -> None:
    ...
    assert "static/css/scan.css?v=20260831-v0.471" in index
    assert "static/js/app.js?v=20260831-v0.471" in index
...
def test_v158_core_page_polish_and_scan_geometry() -> None:
    ...
    expected_asset_keys = {
        "styles": "20260831-v0.471",
        "home": "20260831-v0.471",
        "scan": "20260831-v0.471",
        "racks": "20260831-v0.471",
        "admin": "20260831-v0.471",
    }
...
def test_admin_control_center_modal_structure() -> None:
    index = (ROOT / "index.html").read_text(encoding="utf-8")
    assert 'static/css/admin.css?v=20260831-v0.471' in index
    for element_id in (
        "adminModalEyebrow",
...
def test_v161_scan_timestamp_rack_status_and_rack_control_centers() -> None:
    ...
    assert "static/css/scan.css?v=20260831-v0.471" in index
    assert "static/css/racks.css?v=20260831-v0.471" in index
    assert "static/js/app.js?v=20260831-v0.471" in index
    for element_id in (
...
def test_v168_manual_edit_exact_row_capture_repair() -> None:
    ...
    assert "static/js/app.js?v=20260831-v0.471" in html
...
def test_v169_manual_edit_glass_type_filters() -> None:
    ...
    assert 'static/css/admin.css?v=20260831-v0.471' in html
    assert 'static/js/app.js?v=20260831-v0.471' in html
...
def test_v193_guarded_cross_delivery_date_scanning() -> None:
    ...
    assert 'APPLICATION_VERSION = "472"' in contract
    ...
    assert 'Current maintained release: **v0.471**' in readme
...
def test_v194_exact_manual_scans_result_colors_sound_and_all_scans_history() -> None:
    ...
    assert 'APPLICATION_VERSION = "471"' in contract
...
def test_v197_print_export_document_preview_control_center() -> None:
    ...
    assert 'static/css/print.css?v=20260831-v0.471' in index
    assert 'static/js/app.js?v=20260831-v0.471' in index
    assert 'aria-label="Application version 0.326"' in index
    assert '<strong>0.326</strong>' in index
    assert 'Current maintained release: **v0.471**' in readme
...
def test_v199_multi_order_preset_gui_and_live_preview() -> None:
    ...
    assert "static/css/print.css?v=20260831-v0.471" in html
    assert "static/js/app.js?v=20260831-v0.471" in html
...
def test_v200_live_preview_all_filters_preset_builder_and_output_selector() -> None:
    ...
    assert 'APPLICATION_VERSION = "471"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 11' in contract
    assert 'static/css/styles.css?v=20260831-v0.471' in html
    assert 'static/js/app.js?v=20260831-v0.471' in html
    assert 'Current maintained release: **v0.471**' in readme
...
def test_v204_print_preview_geometry_and_visual_polish() -> None:
    ...
    assert 'static/css/styles.css?v=20260831-v0.471' in html
    assert 'static/js/app.js?v=20260831-v0.471' in html
    assert 'aria-label="Application version 0.471"' in html
    assert '<strong>0.471</strong>' in html
    assert 'APPLICATION_VERSION = "471"' in contract
    assert 'Current maintained release: **v0.471**' in readme
...
def test_v211_letter_preview_parity_and_filter_summary() -> None:
    ...
    assert 'static/css/styles.css?v=20260831-v0.471' in app
...
def test_v215_date_first_branded_header_and_alternating_rows() -> None:
    ...
    assert 'barefoot-company-builders-firstsource-print-logo.png?v=20260831-v0.471' in sheet_markup
...
def test_v218_reliable_print_logo_and_tightened_header():
    ...
    assert 'barefoot-company-builders-firstsource-print-logo.png?v=20260831-v0.471' in markup
...
def test_v219_shared_preview_print_styles_and_portrait_zoom():
    ...
    assert 'function localPrintPackageStylesheetUrls()' in app
    assert 'static/css/styles.css?v=20260831-v0.471' in app
    assert 'static/css/print.css?v=20260831-v0.471' in app
...
def test_v0451_priority_ribbon_multi_term_search_and_transit_transfer() -> None:
    ...
    assert 'APPLICATION_VERSION = "471"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 11' in contract
    assert '20260901-v0.472' in index
    assert 'aria-label="Application version 0.472"' in index
    assert '<strong>0.472</strong>' in index
    assert 'Current maintained release: **v0.472**' in readme
...
def test_v0453_indian_trail_popup_layout_and_glass_transfer_motion() -> None:
    ...
    assert 'APPLICATION_VERSION = "471"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 11' in contract
    assert '20260831-v0.471' in index
    assert 'aria-label="Application version 0.471"' in index
    assert '<strong>0.471</strong>' in index
    assert 'Current maintained release: **v0.471**' in readme
...
def test_v0471_statistics_common_size_selector_recovers_dynamic_options() -> None:
    ...
    assert 'if (!state.homeReportSummary) void loadHomeReportSummary();' in app
    assert 'state.homeChartMetric === "glass-sizes"' in app


def test_v0472_background_production_index_machine_settings_and_order_review_contract() -> None:
    index = (ROOT / "index.html").read_text(encoding="utf-8")
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    admin_css = (ROOT / "static/css/admin.css").read_text(encoding="utf-8")
    scan_css = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    ...
    assert '<strong>0.472</strong>' in index
    assert 'Current maintained release: **v0.472**' in readme
    assert '## v0.472 - Background Production Indexing, Machine Settings, and Order Review' in changelog

    assert 'data-admin-modal="productionFiles"' in index
    assert 'manage_production_files' in app
    assert 'manage_production_files' in store
    assert 'parsed.path == "/api/admin/production-files"' in server
    assert 'parsed.path == "/api/admin/production-files/refresh"' in server
    assert 'productionFileSettingsModalHtmlV472' in app
    assert '.production-settings-shell-v472' in admin_css

    assert 'production-file-index.json' in production_files
    assert 'def refresh_async' in production_files
    assert 'allow_content_read=False' in server
    assert 'assignedMachine' in production_files and 'actualMachine' in production_files
    assert 'machineOverride' in production_files

    assert 'function renderScanOrderGroupsV472' in app
    assert 'data-toggle-scan-order-v472' in app
    assert '.scan-order-group-row-v472' in scan_css
...
def test_v0243_formatted_xlsx_and_raw_csv_exports():
    ...
    assert 'PRINT_XLSX_LOGO_PATH = "static/images/barefoot-company-builders-firstsource-print-logo.png?v=20260831-v0.471"' in app
...
def test_v0245_final_shared_controls_and_requested_action_migration():
    ...
    assert 'static/css/shared-ui.css?v=20260831-v0.471' in html
    assert html.index('static/css/shared-ui.css?v=20260831-v0.471') > html.index('static/css/shell.css?v=20260731-v0.192')
    for button_id in (
        "bayCheckBtn",
        "rackEditOpenBtn",
        "bayManualSubmitBtn",
...
def test_v0247_css_ownership_and_stable_shared_controls():
    ...
    assert 'static/css/print.css?v=20260831-v0.471' in html
    assert 'static/css/racks.css?v=20260831-v0.471' in html
    assert 'static/css/admin.css?v=20260831-v0.471' in html
    assert 'static/css/bays.css?v=20260831-v0.471' in html
    assert 'static/css/shared-ui.css?v=20260831-v0.471' in html
...
def test_v0258_inline_live_statistics_workspace_replaces_modal_and_avoids_duplicate_data():
    ...
    assert 'APPLICATION_VERSION = "471"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 11' in contract
    assert 'static/css/statistics.css?v=20260831-v0.471' in html
    assert 'static/js/app.js?v=20260831-v0.471' in html
    assert 'aria-label="Application version 0.471"' in html
    assert '<strong>0.471</strong>' in html
    assert 'Current maintained release: **v0.471**' in readme
...
def test_v0264_statistics_visibility_scan_selector_and_review_scroll_polish():
    ...
    assert 'CURRENT_SCHEMA_VERSION = 11' in contract
    assert 'static/css/statistics.css?v=20260831-v0.471' in html
    assert 'static/css/scan.css?v=20260831-v0.471' in html
    assert 'static/js/app.js?v=20260831-v0.471' in html
    assert 'aria-label="Application version 0.471"' in html
    assert '<strong>0.471</strong>' in html
    assert 'Current maintained release: **v0.471**' in readme
...
def test_v0271_rack_creation_visuals_deletion_and_bay_map_cleanup():
    ...
    assert 'APPLICATION_VERSION = "471"' in (ROOT / "database" / "contract.py").read_text(encoding="utf-8")
...
def test_v0304_rush_popup_isolation_and_tc22_mobile_ownership():
    ...
    assert 'APPLICATION_VERSION = "471"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 11' in contract
    assert "20260831-v0.471" in html
    assert 'aria-label="Application version 0.326"' in html
...
def test_v0306_single_page_mobile_workflow_and_dialog_repairs():
    ...
    assert 'static/css/mobile.css?v=20260831-v0.471' in html
...
def test_v316_physical_transit_counts_bay_bulk_edit_and_scan_archive_compaction() -> None:
    ...
    assert "static/css/bays.css?v=20260831-v0.471" in index
    assert "static/js/app.js?v=20260831-v0.471" in index
...
def test_v331_scan_page_sort_runtime_and_route_coloring() -> None:
    ...
    assert "const rows = sortScanItems(filteredItems());" in app
    assert "closeScanColumnFilterMenu" not in app
    assert "openScanColumnFilterMenu" not in app
...
def test_v332_rack_set_gradient_route_resolution_and_packing_print_polish() -> None:
    ...
    assert "barefoot-company-builders-firstsource-print-logo.png?v=20260831-v0.471" in operations
    ...
    assert "barefoot-company-builders-firstsource-print-logo.png?v=20260831-v0.471" in server
...
def test_v338_manual_rack_departure_old_bay_snooze_and_investigation_polish() -> None:
    ...
    assert "barefoot-company-builders-firstsource-print-logo.png?v=20260831-v0.471" in report
...
def test_v0372_home_only_header_decoration() -> None:
    ...
    assert "static/css/home.css?v=20260831-v0.471" in index
    assert "Current maintained release: **v0.471**" in readme
...
def test_v330_superseded_dimensions_log_reachability_and_admin_launchers() -> None:
    ...
    assert index.count('data-admin-gui-launch') == 11
...
def test_v353_exact_route_geometry_canonical_glass_and_portable_workflow_guidance() -> None:
    ...
    assert 'static/css/mobile.css?v=20260831-v0.471' in index
...
def test_v0425_global_search_result_card_hierarchy_and_priority_metadata() -> None:
    ...
    assert 'Stage:</b>' in render_block
...
def test_v0428_global_search_consistent_compact_polish() -> None:
    ...
    assert '<b class="global-result-inline-label-v427">Stage:</b>' in render_block
