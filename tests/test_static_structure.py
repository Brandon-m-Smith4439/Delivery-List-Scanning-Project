...
def test_previous_brand_logo_and_main_bay_transit_contract() -> None:
    ...
    assert "static/css/bays.css?v=20260915-v0.540" in html
    assert logo.exists()
...
def test_main_scan_date_wide_layout_and_refresh_contract() -> None:
    ...
    assert 'static/css/scan.css?v=20260917-v0.543' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
    ...
    assert 'return `<tr class="scan-order-group-v477${priorityClass}${newOrderClassV534}" data-order-group-v477="${escapeHtml(order)}"><td colspan="8">' in app
...
def test_v484_aw_reject_persistence_and_direct_sync_contract() -> None:
    ...
    config_path = ROOT / "automation" / "sql_delivery_export" / "sql-export.config.json"
    ...
    assert 'APPLICATION_VERSION = "548"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 21' in contract
    ...
    if config is not None:
        ...
        # Release packages intentionally omit operator-local automation config.
        # The maintained runner owns the same defaults when no local override is present.
        assert 'Get-OptionalProperty -Object $settings -Name "Enabled" -DefaultValue $true' in runner
        assert 'Get-OptionalProperty -Object $settings -Name "IncrementalPastDays" -DefaultValue 30' in runner
        assert 'Get-OptionalProperty -Object $settings -Name "FullPastDays" -DefaultValue 365' in runner
...
def test_v0473_recent_production_index_statistics_and_scan_grouping_contract() -> None:
    ...
    assert 'aria-label="Application version 0.548"' in index
    assert '<strong>0.548</strong>' in index
    assert 'Current maintained release: **v0.548**' in readme
...
def test_single_javascript_bundle_is_loaded() -> None:
    ...
    assert scripts == ["static/js/app.js?v=20260918-v0.546"]
    app = (ROOT / "static" / "js" / "app.js").read_text(encoding="utf-8")
...
def test_admin_control_center_modal_structure() -> None:
    index = (ROOT / "index.html").read_text(encoding="utf-8")
    assert 'static/css/admin.css?v=20260917-v0.544' in index
    for element_id in (
        "adminModalEyebrow",
...
def test_v164_simplified_gui_headers_and_automation_action() -> None:
    ...
    assert "static/js/app.js?v=20260918-v0.546" in index
...
def test_v169_manual_edit_glass_type_filters() -> None:
    ...
    assert 'static/css/admin.css?v=20260917-v0.544' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
...
def test_v203_print_header_date_layout_direct_print_and_exact_preview() -> None:
    ...
    assert 'APPLICATION_VERSION = "548"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 21' in contract
    assert 'static/css/styles.css?v=20260916-v0.542' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
    assert 'Current maintained release: **v0.548**' in readme
...
def test_v211_letter_preview_parity_and_filter_summary() -> None:
    ...
    assert 'static/css/styles.css?v=20260916-v0.542' in app
    ...
    assert 'static/css/styles.css?v=20260916-v0.542' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
    assert 'aria-label="Application version 0.548"' in html
    assert '<strong>0.548</strong>' in html
    assert 'Current maintained release: **v0.548**' in readme
...
def test_v218_reliable_print_logo_and_tightened_header():
    ...
    assert '.delivery-print-sheet-v203 .sheet-location-row.is-long .sheet-location-title' in css
    assert 'APPLICATION_VERSION = "548"' in contract
    assert 'static/css/print.css?v=20260916-v0.542' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
    assert 'Current maintained release: **v0.548**' in readme
...
def test_v219_shared_preview_print_styles_and_portrait_zoom():
    ...
    assert 'function localPrintPackageStylesheetUrls()' in app
    assert 'static/css/styles.css?v=20260916-v0.542' in app
    assert 'static/css/print.css?v=20260916-v0.542' in app
...
def test_v0460_explicit_clear_glass_identity_new_marker_and_static_sheen() -> None:
    ...
    animation_tail = scan_css[scan_css.index('v0.460 Clear-glass/new-marker clarity + static glass-header sheen'):]
    assert animation_tail.count('@keyframes') == 1
    assert '@keyframes scan-page-enter-v529' in animation_tail
...
def test_v0462_compact_glass_header_new_badge_and_static_sheen() -> None:
    ...
    assert v462_css.count('animation:') >= 2
    assert 'scan-page-enter-v529 150ms' in v462_css
    assert 'animation:none !important' in v462_css
...
def test_v0252_uniform_bay_actions_and_sidebar_aligned_primary_buttons():
    ...
    assert 'APPLICATION_VERSION = "548"' in contract
    assert 'static/css/bays.css?v=20260915-v0.540' in html
    assert 'static/css/shared-ui.css?v=20260917-v0.543' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
    assert 'aria-label="Application version 0.548"' in html
    assert '<strong>0.548</strong>' in html
    assert 'Current maintained release: **v0.548**' in readme
...
def test_v0258_inline_live_statistics_workspace_replaces_modal_and_avoids_duplicate_data():
    ...
    assert 'APPLICATION_VERSION = "548"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 21' in contract
    assert 'static/css/statistics.css?v=20260915-v0.540' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
    assert 'aria-label="Application version 0.548"' in html
    assert '<strong>0.548</strong>' in html
    assert 'Current maintained release: **v0.548**' in readme
...
def test_v0271_rack_creation_visuals_deletion_and_bay_map_cleanup():
    ...
    assert 'APPLICATION_VERSION = "548"' in (ROOT / "database" / "contract.py").read_text(encoding="utf-8")
    assert "CURRENT_SCHEMA_VERSION = 21" in (ROOT / "database" / "contract.py").read_text(encoding="utf-8")
...
def test_v316_physical_transit_counts_bay_bulk_edit_and_scan_archive_compaction() -> None:
    ...
    assert "static/css/bays.css?v=20260915-v0.540" in index
    assert "static/js/app.js?v=20260918-v0.546" in index
...
def test_v333_route_history_cross_stage_delete_loading_and_preview_polish() -> None:
    ...
    assert 'Delete this order item from the delivery list?' in app
    assert 'Delete Order Item' in app
    assert 'Delete From All Stages' not in app
...
def test_v355_portable_presentation_preserves_stable_engine_identity() -> None:
    ...
    assert 'class="app-primary-button lookup-save-button-v529" title="Save station display name"' in app
    assert '<span>Save</span>' in app
    ...
    assert 'class="app-primary-button lookup-save-button-v529"' in station_block
    assert '<span>Save</span>' in station_block
...
def test_v0477_scan_progress_order_grouping_and_fast_sketch_contract() -> None:
    ...
    assert 'const chunkSize = ["prewarm", "future-prewarm"].includes(context) ? 40 : 10' in app
...
def test_v498_aw_cutting_progress_and_label_context_contracts() -> None:
    ...
    assert 'current_is_booked = optimization_status in AW_OPTI_STATUS_BOOKED_CODES' in store
...
def test_v499_unified_aw_sync_production_settings_and_bounded_query_contracts() -> None:
    ...
    if config:
        ...
        assert 'Get-OptionalProperty -Object $settings -Name "QueryBatchSize" -DefaultValue 60' in runner
...
def test_v501_piece_cutting_labels_and_fabrication_consistency_contracts() -> None:
    ...
    assert 'downstreamFabricationObserved' in store
    assert 'does not own the A+W Cutting lifecycle' in store
...
def test_v502_manual_sync_observability_cut_evidence_and_compact_label_contracts() -> None:
    ...
    assert 'optimizationPlateCut' in store and 'optimizationPlateStockBooked' in store
    assert '"cutQuantity": float(cut_evidence.get("cutQuantity") or 0)' in store
    assert 'current_is_booked = optimization_status in AW_OPTI_STATUS_BOOKED_CODES' in store
...
def test_v504_recent_cutting_coverage_status_460_and_aw_label_parity_contracts() -> None:
    ...
    if config:
        ...
        assert 'Get-OptionalProperty -Object $settings -Name "OrderLookbackDays" -DefaultValue 14' in runner
...
def test_v505_production_count_email_and_crystal_marker_contracts() -> None:
    ...
    # Production Count is immutable first-import activity rather than mutable
    # review-notice timing or delivery-date inventory. Yield Percentage remains
    # auditable but is excluded from Internal Reject statistics.
    assert 'def is_yield_percentage_reject_reason' in store
    assert "FROM line_update_notices n" in store
    assert "MIN(li.created_at_utc) AS first_seen_at" in store
    assert "def production_first_seen_bounds" in store
    assert 'plant_time_zone' in store
    ...
    assert 'function openTodayProductionGlassAuditV536' in app
    assert 'data-production-glass-detail-v536' in app
    ...
    assert '.statistics-production-glass-drill-v536' in statistics_css
    assert '.statistics-production-audit-v536' in statistics_css
...
def test_v511_scan_cutting_progress_timestamps_reject_ribbon_and_label_contracts() -> None:
    ...
    assert 'const layoutClassV531 = dateWide.previous && dateWide.next ? "is-paired-v486" : "is-single-v531";' in app
    ...
    assert 'static/css/scan.css?v=20260917-v0.543' in index
    assert 'static/css/shared-ui.css?v=20260917-v0.543' in index
    assert 'static/css/mobile.css?v=20260909-v0.517' in index
...
def test_v512_scan_reentry_order_detail_filters_and_bay_animation_contracts() -> None:
    ...
    # v0.534 keeps the single Status completion model but replaces the old
    # Machine group with direct production/scanner Progress checkpoint filters.
    ...
    for token in ('progress-cutting', 'progress-waterjet', 'progress-denver', 'progress-staging', 'progress-outbound'):
        assert f'data-filter="{token}"' in index
    for retired in ('machine-no-fab', 'machine-waterjet', 'machine-denver'):
        assert f'data-filter="{retired}"' not in index
...
def test_v514_scan_search_order_detail_and_production_reporting_contracts() -> None:
    ...
    assert '<strong>0.548</strong>' in index
    for asset in (
        'static/css/styles.css?v=20260916-v0.542',
        'static/css/statistics.css?v=20260915-v0.540',
        'static/css/scan.css?v=20260917-v0.543',
        'static/css/shared-ui.css?v=20260917-v0.543',
        'static/js/app.js?v=20260918-v0.546',
    ):
    ...
    # One completion model remains authoritative while v0.534 exposes the visible
    # production/scanner checkpoints through the dedicated Progress filter group.
    assert 'function scanWorkflowCompletionStateV514' in app
    completion = app[app.index('function scanWorkflowCompletionStateV514'):app.index('function isScanFilterActive')]
    assert 'scanDateWideProgressStepsV485(item)' in completion
    assert 'progress-waterjet' in app and 'progress-denver' in app and 'progress-staging' in app
    assert 'const allComplete = required.every((step) => Boolean(step.complete));' in completion
    assert 'if (allComplete) return "complete";' in completion
...
def test_v513_smart_search_external_remake_filters_and_badges_contracts() -> None:
    ...
    for asset in (
        'static/css/styles.css?v=20260916-v0.542',
        'static/css/scan.css?v=20260917-v0.543',
        'static/css/admin.css?v=20260917-v0.544',
        'static/css/bays.css?v=20260915-v0.540',
        'static/css/shared-ui.css?v=20260917-v0.543',
    ...
    assert 'return self.attach_cutting_search_states_v513(matched_results[:20])' in store
...
def test_v515_progress_layout_label_width_and_sketch_recovery_contracts() -> None:
    ...
    for asset in (
        ...
        assert asset in index
    assert 'static/css/statistics.css?v=20260915-v0.540' in index
...
def test_v518_manual_fabrication_and_compact_order_details_contracts() -> None:
    ...
    assert 'cutting = dict(label_hints.get(request_key) or {})' in server
    assert 'candidates.push({' in app
    assert 'key, order, item, job, product, lastRejectedAt,' in app
    assert 'deliveryDate: String(row?.deliveryDate || state.meta?.deliveryDate || "")' in app
...
def test_v521_machine_library_fabrication_warm_cache_and_order_controls_contracts() -> None:
    ...
    for asset in (
        'static/css/scan.css?v=20260917-v0.543',
        'static/css/print.css?v=20260916-v0.542',
        'static/css/shared-ui.css?v=20260917-v0.543',
        'static/js/app.js?v=20260918-v0.546',
    ):
    ...
    assert 'fabricationDeliveryCacheLimitV521: FUTURE_FABRICATION_CACHE_LIMIT_V530' in app
...
def test_v522_durable_cutting_and_fabrication_progress_monitor_contracts() -> None:
    ...
    assert 'data-refresh-progress-v543' in app
    assert 'refreshOrderItemProgressV543' in app
...
def test_v523_lifecycle_aware_production_checks_and_order_detail_cache_contracts() -> None:
    ...
    assert 'CURRENT_SCHEMA_VERSION = 21' in contract
    assert 'Current maintained release: **v0.548**' in readme
    assert '## v0.527 - Stable Production Checks, A+W Review Flags, and Faster Navigation' in changelog
    ...
    assert 'version="v540-aw-production-7"' in runner
...
def test_v524_inventory_page_api_schema_and_responsive_contract() -> None:
    ...
    assert 'APPLICATION_VERSION = "548"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 21' in contract
    assert 'static/css/inventory.css?v=20260918-v0.548' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
...
def test_v527_stable_production_checks_aw_review_and_navigation_contracts() -> None:
    ...
    assert 'is-hover-suppressed-v527' not in app and 'is-hover-suppressed-v527' not in styles_css
...
def test_v528_codex_completion_polish_and_regression_contracts() -> None:
    app = (ROOT / "static" / "js" / "app.js").read_text(encoding="utf-8")
    scan_css = (ROOT / "static" / "css" / "scan.css").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    contract = (ROOT / "database" / "contract.py").read_text(encoding="utf-8")
    ...
    assert 'CURRENT_SCHEMA_VERSION = 21' in contract
    assert 'Current maintained release: **v0.548**' in readme
    assert changelog.startswith('## v0.548 - Inventory Action Accessibility and Control Alignment')
    assert 'static/css/scan.css?v=20260917-v0.543' in html
    assert 'static/js/app.js?v=20260918-v0.546' in html
    assert 'static/css/inventory.css?v=20260918-v0.548' in html

    # Visible whole-list editing no longer exposes obsolete stage terminology.
    assert 'confirmLabel: "Delete Order Item"' in app
    assert 'Delete From All Stages' not in app
    assert 'deleted from ${deletedCount} synchronized record' in app

    # Inventory/Manual Entry exact-match Spanish labels and English admin loading/error
    # paths are maintained explicitly rather than relying on accidental translation.
    for phrase in (
        '["Item Number", "Numero de articulo"]',
        '["Item ID", "ID de articulo"]',
        '["Custom Glass Type", "Tipo de vidrio personalizado"]',
        '["Custom Item ID", "ID de articulo personalizado"]',
        '["SQFT / piece", "Pies cuadrados / pieza"]',
        '["Total SQFT", "Pies cuadrados totales"]',
        '["Awaiting physical scan", "Esperando escaneo fisico"]',
        '["System item missing physically", "Articulo del sistema faltante fisicamente"]',
    ):
        assert phrase in app
    loading = app[app.index('function adminModalLoadingHtmlV507'):app.index('function finishDeferredAdminModalOpenV507')]
    assert 'appLoadingStateHtmlV529("Loading settings..."' in loading
    assert 'localizedUiValue("Settings could not be loaded.")' in loading

    # Internal Reject visual age and per-user review state are separate; the
    # compact one-row ribbon remains readable after the v0.527 visual reduction.
    assert 'function renderRejectReviewControlV529' in app
    assert 'scanRejectReviewControlV529' in app
    tail = scan_css[scan_css.rindex('/* v0.527: priority/reject rows'):]
    assert 'min-height: 31px !important' in tail
    assert 'font-size: 10px !important' in tail
    assert 'font-size: 7.8px !important' in tail



def test_v529_review_progress_machine_loading_and_visual_contracts() -> None:
    app = (ROOT / "static" / "js" / "app.js").read_text(encoding="utf-8")
    scan = (ROOT / "static" / "css" / "scan.css").read_text(encoding="utf-8")
    print_css = (ROOT / "static" / "css" / "print.css").read_text(encoding="utf-8")
    shared = (ROOT / "static" / "css" / "shared-ui.css").read_text(encoding="utf-8")
    ...
    assert 'CURRENT_SCHEMA_VERSION = 21' in (ROOT / "database" / "contract.py").read_text(encoding="utf-8")
    assert 'internal_reject_review_receipts' in migrations
    assert 'manual_production_progress_overrides' in migrations
    assert '_ensure_column(connection, "inventory_scans", "delivery_date", "TEXT NOT NULL DEFAULT' in migrations
    assert 'def acknowledge_internal_rejects' in operations
    assert '/api/operations/internal-rejects/acknowledge' in (ROOT / "server.py").read_text(encoding="utf-8")
    assert 'scanRejectReviewControlV529' in app
    assert 'is-reject' in scan

    assert 'def advance_delivery_progress' in store
    assert 'manualMachineComplete' in store
    assert 'target.startswith("machine:")' in store
    assert 'lastRejectedAt' in store[store.index('def aw_fabrication_hints_for_requests'):store.index('def get_order_detail')]
    assert 'Cutting complete' in app and 'machine:${machine.code}' in app
    header = app[app.index('class="manual-edit-card-title-line"'):app.index('class="manual-edit-card-quantity"')]
    assert header.count('manual-edit-card-progress-v527') == 1
    assert 'manual-edit-scan-status' not in header

    priority = scan[scan.rindex('/* v0.529 priority rows'):]
    assert 'has-priority-rail-v482 > td:nth-child(n)' in priority
    assert 'background-image: none !important' in priority
    assert 'is-priority-rush-v480 { --priority-wash-v529: #b42336; }' in priority
    assert 'is-priority-remake-v480 { --priority-wash-v529: #23272d; }' in priority
    assert 'background: linear-gradient(100deg, #821e35' in priority
    assert 'white-space: nowrap !important' in print_css[print_css.rindex('v0.529 Print / Export'):]

    assert 'function appLoadingStateHtmlV529' in app
    assert app.count('appLoadingStateHtmlV529(') >= 12
    assert '.app-loading-bar-v529::after' in shared
    assert 'prefers-reduced-motion:reduce' in shared
    assert '.production-order-item-v476.production-order-item-v480.production-order-item-v481.production-order-item-v512.production-order-item-v518 .production-item-main-v519' in shared
    assert 'grid-column:3 !important;' in shared[shared.rindex('/* Override the legacy <=1740px'):]
    for asset in (
        'static/css/rejects.css?v=20260914-v0.530',
        'static/css/scan.css?v=20260917-v0.543',
        'static/css/inventory.css?v=20260918-v0.548',
        'static/css/admin.css?v=20260917-v0.544',
        'static/css/print.css?v=20260916-v0.542',
        'static/css/shared-ui.css?v=20260917-v0.543',
        'static/js/app.js?v=20260918-v0.546',
    ...
    machine_normalize = store[store.index('def _normalize_production_file_settings'):store.index('def get_machine_configuration')]
    assert 'True if code in {"denver", "waterjet"}' in machine_normalize
    assert 'editingSystemMachine' in app
    assert 'Evidence-backed machines remain available' in app
    assert 'glassProfileSheetSizeV529' in app and 'glassProfileSheetEmailsV529' in app
    assert 'inventoryManualDeliveryDate' in app


def test_v530_shared_attention_review_date_alerts_and_future_prewarm_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    index = (ROOT / "index.html").read_text(encoding="utf-8")
    scan = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    admin = (ROOT / "static/css/admin.css").read_text(encoding="utf-8")
    ...
    assert '--attention-new-order: #1766D8' in styles
    assert '--attention-internal-reject: #F28C28' in styles
    assert '--attention-external-remake: #111111' in styles
    assert '--attention-rush: #C62828' in styles
    assert 'custom-select-indicator-dot-v530.is-reject' in styles
    assert 'custom-select-indicator-dot-v530.is-remake' in styles
    assert 'custom-select-indicator-dot-v530.is-rush' in styles
    assert 'width: 260px !important' in styles

    assert 'scan-review-grid-v530' in index
    for control in ('scanUpdateReviewControl', 'scanRemakeReviewControlV530', 'scanRejectReviewControlV529', 'scanRushReviewControlV530'):
        assert f'id="{control}"' in index
    assert 'display: flex !important;' in scan[scan.rindex('v0.530 Scan attention palette'):]
    assert 'overflow-x: auto !important;' in scan[scan.rindex('v0.530 Scan attention palette'):]

    assert 'insertTab("lookup:color_manager", "Color Manager")' in app
    assert 'attentionColorManagerHtmlV530' in app
    assert '<section class="color-manager-attention-v539">${attentionColorManagerHtmlV530()}</section>' in app
    assert 'data-attention-color-save' in app
    assert 'type: "attention_color"' in app
    assert 'ATTENTION_COLOR_DEFAULTS_V530' in store
    assert 'lookup_type == "attention_color"' in store
    assert 'attention-color-row-v530' in admin

    assert 'pendingOrderCount' in operations
    assert 'pendingRemakeCount' in operations
    assert 'pendingRushCount' in operations
    assert 'review_kind=""' in operations or 'review_kind: str = ""' in operations
    assert 'reviewKind: "order"' in app
    assert 'reviewKind: kind' in app
    assert 'attentionSummary' in store
    assert 'New Internal Rejects' in app and 'New External Remakes' in app and 'New Rushes' in app

    assert 'scheduleFutureFabricationPrewarmV530' in app
    assert 'date >= today' in app
    assert '{ background: true }' in app
    assert 'context: background ? "future-prewarm" : "prewarm"' in app
    assert 'context !== "future-prewarm"' in app
    assert '.is-attention-remake' in print_css and 'var(--attention-external-remake)' in print_css


def test_v531_review_isolation_fabrication_progress_and_import_feedback_contracts() -> None:
    app = (ROOT / "static" / "js" / "app.js").read_text(encoding="utf-8")
    scan_css = (ROOT / "static" / "css" / "scan.css").read_text(encoding="utf-8")
    styles_css = (ROOT / "static" / "css" / "styles.css").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    store = (ROOT / "backend" / "store.py").read_text(encoding="utf-8")
    manifest = (ROOT / "sounds" / "audio_manifest.json").read_text(encoding="utf-8")
    contract = (ROOT / "database" / "contract.py").read_text(encoding="utf-8")

    assert 'APPLICATION_VERSION = "548"' in contract
    assert 'CURRENT_SCHEMA_VERSION = 21' in contract
    assert 'excludedFiltersV531: new Set()' in app
    assert 'function applyScanReviewPresetV531' in app
    assert 'order: { include: "updated", exclude: [] }' in app
    assert 'reject: { include: "internal-rejects", exclude: [] }' in app
    assert 'remake: { include: "remakes", exclude: [] }' in app
    assert 'rush: { include: "rushes", exclude: [] }' in app
    assert 'function toggleScanExcludeFilterV531' in app
    assert 'function acknowledgeAllReviewsV531' in app
    assert 'invalidateScanDateBundleV526(String(state.meta?.deliveryDate || ""))' in app
    assert 'window.DLSLineUpdates.applyPayload(activeRecordV531.flags' in app
    assert 'scan-filter-exclude-section-v531' not in html
    assert 'id="scanReviewAllBtnV531"' in html
    assert 'custom-select-indicator-dot-v530 is-remake scan-review-indicator-v531' in html
    assert 'id="scanFabricationLoadV531"' in html
    assert 'function setFabricationLoadProgressV531' in app
    assert '.scan-progress-flow-v485.is-single-v531' in scan_css
    assert 'min-height: 23px !important;' in scan_css
    assert 'td.scan-glass-type-cell-v484' in scan_css
    assert 'transform 210ms cubic-bezier' in styles_css
    assert 'import_success: "sounds/import_success.wav"' in app
    assert 'import_failed: "sounds/import_failed.wav"' in app
    assert '"import_success"' in manifest and '"import_failed"' in manifest
    assert 'if current_remake and (change_type == "new" or not previous_remake):' in store
    assert 'window_sql += " AND datetime(created_at) >= datetime(?)"' in store
    assert 'No new review items were imported.' in store


def test_v532_numeric_cutting_unified_exclusion_fab_and_priority_visual_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    scan_css = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    shared_css = (ROOT / "static/css/shared-ui.css").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    ...
    # Cutting keeps the v0.532 numeric piece convention.
    progress = app[app.index('function progressStepHtmlV475'):app.index('function scanProgressMarkupV475')]
    assert 'const value = step.kind === "no-fab" ? "N/A" : `${scanned}/${qty}`;' in progress
    assert 'step.kind === "cutting" && step.detail' not in progress

    # v0.533 removes user-facing exclusion mode from both filter surfaces.
    assert 'id="scanFilterModeSwitchV532"' not in html
    assert 'id="printFilterModeSwitchV532"' not in html
    assert 'order: { include: "updated", exclude: [] }' in app
    assert 'if (filter === "updated") return isNewOrUpdatedItem(item) && String(item.updateReviewKind || "order") === "order";' in app
    assert 'function printFilterModeIsExcludeV532()' in app
    print_mode = app[app.index('function printFilterModeIsExcludeV532()'):app.index('function syncPrintFilterModeUiV532()')]
    assert 'return false;' in print_mode

    # FAB state persists across refresh/tab transitions, waits one second
    # before revealing, and moves Scan smoothly when it appears/disappears.
    assert 'FABRICATION_LOAD_DELAY_MS_V533 = 1000' in app
    assert 'FABRICATION_MEMORY_STORAGE_KEY_V533' in app
    assert 'restoreFabricationMemoryV533();' in app
    assert 'scheduleFabricationMemorySaveV533();' in app
    assert 'setFabricationLoadVisibleV533' in app
    v533_scan = scan_css[scan_css.rindex('v0.533 review controls')-6:]
    assert 'max-height 320ms cubic-bezier' in v533_scan
    assert '.scan-fabrication-load-v531.is-visible-v533' in v533_scan
    assert 'height: 10px !important;' in v533_scan
    assert '#176fd0 0%,#199bb7 52%,#35ad69 100%' in v533_scan
    assert '.app-loading-track-v533' in shared_css
    assert '#176fd0 0%,#1b98b8 52%,#35ad69 100%' in shared_css

    # IR spacing is +30% over v0.532 and its ribbon-to-piece orange is stronger.
    assert 'gap: 26px !important;' in v533_scan
    assert 'var(--attention-internal-reject) 35%, transparent' in v533_scan
    assert 'var(--attention-internal-reject) 20%, transparent' in v533_scan

    # External Remake black belongs to the row. The TD is transparent and the
    # lookup-derived glass surface is opaque above it so black cannot tint it.
    assert '.scan-order-item-v477.is-priority-remake-v480 {' in v533_scan
    assert 'rgba(10,12,15,.42) 0%' in v533_scan
    assert '.scan-order-item-v477.is-priority-remake-v480 > td:nth-child(n)' in v533_scan
    assert 'background: transparent !important;' in v533_scan
    assert 'color-mix(in srgb, var(--glass-type-color) 28%, #fff)' in v533_scan


def test_v533_superseded_search_review_and_loading_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    operations = (ROOT / "backend/operations.py").read_text(encoding="utf-8")
    scan_css = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    ps = (ROOT / "automation/sql_delivery_export/Run-DeliveryListSqlAutomation.ps1").read_text(encoding="utf-8")

    # Strict same-day normal -> External Remake duplicate candidate; operator
    # superseded review remains the only deletion path.
    assert 'Get-ExactOrderSignature' in ps
    assert "rule = 'v0.535-same-day-normal-remake-exact-duplicate-2'" in ps
    assert 'sameDeliveryDate = $true' in ps
    assert 'sameJobNumber = $true' in ps
    assert 'sameCustomer = $true' in ps
    assert 'sameRoute = [bool]$sameRoute' in ps
    assert 'routeIgnoredForDuplicateIdentity = $true' in ps
    assert 'exactItemSetMatch = $true' in ps
    assert 'originalIsRemake = $false' in ps
    assert 'replacementIsRemake = $true' in ps

    # Review buckets are disjoint; Remakes cannot appear as ordinary updated orders.
    assert 'if "REMAKE" in text or re.search(r"\\bRM\\b", text):' in operations
    assert 'return "remake"' in operations
    assert 'return "order"' in operations
    assert 'id="scanFilterModeSwitchV532"' not in html
    assert 'id="printFilterModeSwitchV532"' not in html
    for button_id in (
        'scanRemakeReviewBtnV530', 'scanRemakeMarkReviewedBtnV530',
        'scanRejectReviewBtnV529', 'scanRejectMarkReviewedBtnV529',
        'scanRushReviewBtnV530', 'scanRushMarkReviewedBtnV530',
    ):
        segment = html[html.index(f'id="{button_id}"')-240:html.index(f'id="{button_id}"')+520]
        assert '<svg' in segment
    assert 'font-size: 7.5px !important;' in scan_css

    # Search includes non-deleted inactive/completed history and limits expensive
    # scan-event work to candidate IDs before relevance/newest sorting.
    assert "AND dl.status <> 'deleted'" in store
    assert 'WITH {priority_cte} candidate_ids AS (' in store
    assert 'JOIN candidate_ids candidate ON candidate.id = se.line_item_id' in store
    assert 'priority_only = not sql_terms' in store
    assert 'include_audit_text = len(sql_terms) > 1' in store
    assert 'matched_results.sort(key=relevance, reverse=True)' in store
    assert 'return (score, date_key, order_rank, item_rank)' in store
    assert 'if (error?.name !== "AbortError") showInlineError(error.message);' in app


def test_v534_blue_x_progress_filter_and_scan_polish_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    scan_css = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    admin_css = (ROOT / "static/css/admin.css").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    ...
    # Blue-X cancellation evidence is vector/annotation based and surfaced in
    # Superseded Orders with item-specific Cutting/scanner production evidence.
    assert '_looks_like_blue_cancellation_x_v534' in production
    assert '_pdf_page_has_cancellation_x_v534' in production
    assert '"sketchCancelled": sketch_cancelled' in production
    assert '_superseded_sketch_safety_v534' in store
    assert 'scannerAfterCancelQty' in store
    assert 'suggestedRemoveOrderNumber' in store
    assert 'superseded-sketch-safety-v534' in admin_css
    assert 'Blue-X cancellation detected on order' in app

    # Machine has been replaced by progress checkpoint filtering; route buttons
    # remain data-aware and are hidden when the selected delivery list has none.
    assert 'id="scanProgressFilterOptionsV534"' in html
    assert 'aria-label="Progress filters"' in html
    assert 'Machine filters' not in html
    for key in ('progress-cutting', 'progress-waterjet', 'progress-denver', 'progress-staging', 'progress-outbound'):
        assert key in app and key in html
    assert 'button.hidden = count === 0;' in app
    assert 'document.querySelectorAll(".route-filter-tab")' in app

    # New orders are identified on the grouped header; FAB uses a true full-row
    # progress track and progress chips have room for labels plus 0/1 counts.
    assert 'scan-order-group-new-v534' in app and 'scan-order-group-new-v534' in scan_css
    fab = html[html.index('id="scanFabricationLoadV531"'):html.index('id="scanReviewGridV530"')]
    assert fab.index('scan-fabrication-load-copy-v531') < fab.index('scanFabricationLoadPercentV531') < fab.index('scan-fabrication-load-track-v531')
    assert 'grid-column: 1 / -1 !important;' in scan_css
    assert 'flex: 0 0 124px !important;' in scan_css

    # External Remake black belongs behind the row. Its Glass Type identity
    # recreates the normal white-backed lookup gradient with dark text.
    v534_scan = scan_css[scan_css.rindex('v0.534 review headers'): ]
    assert 'rgba(10,12,15,.34) 0%' in v534_scan
    assert 'color-mix(in srgb, var(--glass-type-color) 5.2%, #fff)' in v534_scan
    assert 'color: #153a5a !important;' in v534_scan
    assert 'Mark All Reviewed' in html


def test_v535_remake_progress_and_superseded_strengthening_contracts() -> None:
    contract = (ROOT / "database/contract.py").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    scan_css = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    ps = (ROOT / "automation/sql_delivery_export/Run-DeliveryListSqlAutomation.ps1").read_text(encoding="utf-8")
    ...
    v535_scan = scan_css[scan_css.rindex('v0.535 remake glass identity'): ]
    assert '.scan-glass-type-copy-v484 {' in v535_scan
    assert 'color: #08131d !important;' in v535_scan
    assert 'grid-template-columns: minmax(31px,auto) minmax(66px,1fr) 17px' in v535_scan
    assert '> strong {' in v535_scan and 'grid-column: 1 !important;' in v535_scan
    assert '> b {' in v535_scan and 'grid-column: 2 !important;' in v535_scan
    assert 'grid-column: 3 !important;' in v535_scan
    assert 'font-size: 21px !important;' in v535_scan

    assert 'Group-Object { "$($_.job)|$($_.customer)|$($_.exactSignature)" }' in ps
    assert 'routeIgnoredForDuplicateIdentity = $true' in ps
    assert "rule = 'v0.535-same-day-normal-remake-exact-duplicate-2'" in ps
    duplicate_block = ps[ps.index('# Route intentionally stays out of the duplicate identity.'):ps.index('return @($candidates.ToArray())')]
    assert 'foreach ($remake in @($remakeOrders | Sort-Object order))' in duplicate_block
    assert 'Select-Object -First 1' not in duplicate_block
    assert '$sameRoute = ([string]$normal.route' in duplicate_block


def test_v537_inventory_new_ir_navigation_and_export_contracts() -> None:
    contract = (ROOT / "database/contract.py").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    styles = (ROOT / "static/css/styles.css").read_text(encoding="utf-8")
    ...
    # One scanner trigger is one physical pane. The existing aggregate row is
    # incremented only until the frozen target quantity has been satisfied.
    inventory = store[store.index('def record_inventory_scan'):store.index('def record_inventory_manual_entry')]
    assert 'next_qty = counted_qty + 1' in inventory
    assert 'if counted_qty >= target_qty:' in inventory
    assert '"qtyDelta": 1' in inventory
    assert 'BEGIN IMMEDIATE' in inventory
    assert '"remainingQty": remaining_qty' in inventory
    undo = store[store.index('def remove_inventory_scan'):store.index('def _inventory_xlsx_column_name')]
    assert 'if current_qty > 1:' in undo
    assert 'next_qty = current_qty - 1' in undo
    assert 'inventory_scan_piece_removed' in undo

    # Partial open counts remain pending rather than appearing as errors.
    assert 'status_counts = {"matched": 0, "mismatch": 0, "partial": 0' in store
    assert 'status = "partial"' in store
    reconcile = app[app.index('function inventoryRenderReconciliationV524'):app.index('function inventoryRenderTotalsV524')]
    assert 'row.status === "partial"' in reconcile
    assert 'Partially counted' in reconcile

    # The operator-facing IR count uses true new-event identity, never mirror writes.
    assert 'summary["internalRejects"] = max(0, int(reject_sync.get("newInternalRejects") or 0))' in store
    assert '"newInternalRejects": new_internal_rejects' in store
    assert 'existing_event_keys' in store

    # Inventory exports use readable local time, frozen headers, and fitted widths.
    export_start = store.index('def export_inventory_xlsx')
    export = store[export_start:store.index('def export_csv', export_start)]
    assert 'Scanned Date/Time (ET)' in export
    assert 'readable_inventory_time' in export
    assert 'pane ySplit="1"' in export
    assert 'customWidth="1"' in export

    # Sidebar navigation no longer force-collapses under a hovering pointer, and
    # the Old Bays notice cannot leak across page/modal boundaries.
    assert 'is-hover-suppressed-v527' not in app
    assert 'is-hover-suppressed-v527' not in styles
    show_start = app.index('function showPage(page)')
    show_page = app[show_start:show_start + 12000]
    assert 'if (pageChanged) closeOldBayReviewNotice();' in show_page
    modal = app[app.index('function updateModalScrollLock()'):app.index('let modalScrollLockObserver')]
    assert 'if (modalIsOpen) closeOldBayReviewNotice();' in modal


def test_v0538_statistics_calendar_daily_production_and_progress_readability_contracts():
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    app = (ROOT / "static" / "js" / "app.js").read_text(encoding="utf-8")
    scan_css = (ROOT / "static" / "css" / "scan.css").read_text(encoding="utf-8")
    store = (ROOT / "backend" / "store.py").read_text(encoding="utf-8")

    assert '<option value="production-count-daily">Production count by day</option>' not in html
    assert 'static/css/scan.css?v=20260917-v0.543' in html
    assert 'statisticsCalendarAwaitingRangeEndV538' in app
    assert r'/^\d{4}-\d{2}-\d{2}$/' in app
    assert r'/^\d{4}-\d{0}-\d{0}$/' not in app
    assert 'state.statisticsCalendarDraftEnd = key;' in app
    assert 'metric === "production-count-daily"' in app
    assert 'report.productionActivity?.newProduction?.byDate' in app
    assert 'activeReport && Array.isArray(activeReport.incompleteByDeliveryList)' in app
    assert 'Object.prototype.hasOwnProperty.call(activeReport, "rangeRemakeQty")' in app
    assert 'font-size: 12.4px !important' in scan_css
    assert 'grid-template-columns: 29px minmax(0,1fr) 19px !important' in scan_css
    assert 'def plant_timestamp_clause(column: str)' in store
    assert '"byDate": by_date' in store
    assert 'datetime.now(plant_time_zone()).date().replace(day=1)' in store


def test_v0539_inventory_manual_quantity_and_scan_feedback_contracts() -> None:
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    inventory_css = (ROOT / "static/css/inventory.css").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    ...
    manual_block = store[store.index('def record_inventory_manual_entry'):store.index('def complete_inventory_session')]
    assert 'known_target_qty' in manual_block
    assert 'next_qty = counted_qty + qty_delta' in manual_block
    assert 'if known_target_qty and next_qty > known_target_qty:' in manual_block
    assert 'Manual quantity would exceed the system quantity' in manual_block
    assert '"systemPresence": presence' in manual_block

    scan_block = store[store.index('def record_inventory_scan'):store.index('def record_inventory_manual_entry')]
    assert '_inventory_location_presence_for_group_con' in scan_block
    assert '"systemPresence": system_presence' in scan_block
    assert 'Expected quantity already counted' in scan_block

    assert 'function inventorySystemPresenceHtmlV539' in app
    assert 'function inventoryScanFailureV539' in app
    assert 'Inventory scan successful:' in app
    assert 'Inventory scan blocked:' in app
    assert 'Manual inventory count saved:' in app
    assert 'inventory-system-presence-pill-v539' in inventory_css
    assert '.is-present' in inventory_css and '.is-absent' in inventory_css


def test_v0539_inventory_manual_delta_and_safe_undo_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    assert 'if (els.inventoryManualQty) els.inventoryManualQty.value = "1";' in app
    assert '${Number(item.qty || 0) > 1 ? "Undo 1" : "Remove"}' in app
    assert 'if current_qty > 1:' in store
    assert 'Manual Entry and barcode scanning now share one quantity row.' in store


def test_v540_physical_sheet_superseded_refresh_and_compact_progress_contracts() -> None:
    contract = (ROOT / "database" / "contract.py").read_text(encoding="utf-8")
    store = (ROOT / "backend" / "store.py").read_text(encoding="utf-8")
    server = (ROOT / "server.py").read_text(encoding="utf-8")
    runner = (ROOT / "automation" / "sql_delivery_export" / "Run-DeliveryListSqlAutomation.ps1").read_text(encoding="utf-8-sig")
    ...
    app = (ROOT / "static" / "js" / "app.js").read_text(encoding="utf-8")
    scan = (ROOT / "static" / "css" / "scan.css").read_text(encoding="utf-8")
    statistics = (ROOT / "static" / "css" / "statistics.css").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    ...
    assert 'AW_OPTIMIZATION_PLATE_SNAPSHOT_METADATA_KEY' in store
    assert 'Current A+W PROD_OPTI_PLATES rows, one distinct PLATENR per optimization' in store
    assert 'optimization_plates: Iterable[dict[str, Any]] | None = None' in store
    assert 'optimizationPlates' in importer
    assert 'version="v540-aw-production-7"' in runner
    assert 'p.LENGTH AS LengthUnits' in runner and 'p.HEIGHT AS HeightUnits' in runner

    assert 'def detect_superseded_order_candidates_from_scanner' in store
    assert 'v0.540-current-scanner-exact-duplicate-3' in store
    assert '/api/admin/superseded-order-reviews/refresh' in server
    assert 'data-superseded-refresh-v540' in app

    scan_progress = app[app.index('function scanDateWideProgressStepsV485'):app.index('function scanDateWideProgressPairV486')]
    search_progress = app[app.index('function globalSearchProgressStepsV513'):app.index('function globalSearchProgressTextV475')]
    order_detail = app[app.index('function orderDetailProgressV476'):app.index('function normalizedOrderDetailItemV477')]
    assert 'noFabProgressStepV512' not in scan_progress
    assert 'noFabProgressStepV512' not in search_progress
    assert 'noFabProgressStepV512' in order_detail
    final_scan = scan[scan.rindex('v0.540 compact, color-faithful progress'):]
    assert 'height: 32px !important;' in final_scan
    assert 'font-size: 11.25px !important;' in final_scan
    assert 'color: var(--progress-step-color) !important;' in final_scan
    assert 'background: #fff0ad !important;' in final_scan
    assert 'color: #111827 !important;' in final_scan

    assert 'statistics-production-glass-columns-v540' in app
    assert '<span>Stock sheets</span><span>Delivery</span>' in app
    assert 'statistics-production-sheet-cell-v540' in statistics
    assert 'globalSearchAbortControllerV540' in app
    assert 'globalSearchCacheV540' in app
    assert 'core_search_corpus' in store


def test_v541_fab_date_switch_override_bay_and_zoom_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    scan = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    ...
    assert 'FABRICATION_LOAD_DELAY_MS_V533 = 1000' in app
    assert 'state.productionProgressCheckedV522.delete(key);' in app
    assert 'state.cuttingStatusCacheV522.delete(key);' in app
    assert 'fabricationRowsNeedRefreshV527(source)' in app
    assert 'if (forceCheck || foreground) return run();' in app
    assert 'function refreshVisibleScanProgressV540(keys = [])' in app
    assert 'refreshVisibleScanProgressV540(publishedKeysV540)' in app
    assert 'fabricationOverride: Boolean(options.fabricationOverride)' in app
    assert 'Override and scan' in app

    assert 'fabricationOverrideRequired' in store
    assert 'def record_fabrication_scan_override' in store
    assert 'staging_fabrication_override' in store
    assert 'def active_bay_assignment_rows' in store
    assert 'assignments_by_bay' in store

    pending_fab = scan[
        scan.index('/* Denver/WaterJet pending fabrication'):
        scan.index('/* ========================================================================== */', scan.index('/* Denver/WaterJet pending fabrication'))
    ]
    assert '--progress-step-color: var(--progress-left)' not in pending_fab
    authoritative_fab = scan[
        scan.index('/* v0.482 fabrication colors are authoritative'):
        scan.index('@media (max-width: 760px)', scan.index('/* v0.482 fabrication colors are authoritative'))
    ]
    assert '--progress-step-color: var(--progress-left)' not in authoritative_fab
    final_scan = scan[scan.rindex('v0.541 progress scale'):]
    assert 'justify-content: center !important;' in final_scan
    assert 'flex: 0 0 119px !important;' in final_scan
    assert 'flex-basis: 103px !important;' in final_scan
    assert 'font-size: 11.75px !important;' in final_scan
    assert 'min-width: 100px !important;' in final_scan
    assert 'min-width: 94px !important;' in final_scan

    assert 'const critical = Promise.all([' in app
    assert 'Promise.allSettled([' in app
    assert 'scheduleBayTransitAnimationV540' in app



def test_v542_zoom_today_fab_print_and_statistics_refresh_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    styles = (ROOT / "static/css/styles.css").read_text(encoding="utf-8")
    scan = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    print_css = (ROOT / "static/css/print.css").read_text(encoding="utf-8")
    ...
    # Zoom: normal desktop remains the base cascade; only short desktop
    # viewports receive scroll/compact ownership.
    zoom_sidebar = styles[styles.rindex('v0.542 short desktop viewport navigation'):]
    assert '@media (min-width: 961px) and (max-height: 860px)' in zoom_sidebar
    assert 'overflow-y: auto !important;' in zoom_sidebar
    assert 'height: 100dvh !important;' in zoom_sidebar
    assert '@media (min-width: 961px) and (max-height: 680px)' in zoom_sidebar
    zoom_scan = scan[scan.rindex('v0.542 zoom-safe Scan layout'):]
    assert 'grid-template-columns: minmax(0, 1fr) minmax(340px, 360px) !important;' in zoom_scan
    assert 'grid-template-columns: minmax(0, 1fr) 330px !important;' in zoom_scan
    assert 'min-width: 760px !important;' in zoom_scan

    # Review button gets the larger action share and IR rows get edge-only cues.
    assert 'minmax(0, .84fr) minmax(0, 1.16fr)' in zoom_scan
    assert 'font-size: 9.5px !important;' in zoom_scan
    assert 'inset 3px 0 0 color-mix' in zoom_scan
    assert 'inset -3px 0 0 color-mix' in zoom_scan

    # Today FAB can refresh missing evidence without turning every date into
    # frequent production-share work.
    assert 'FABRICATION_TODAY_RETRY_MS_V542 = 60 * 1000' in app
    assert 'function fabricationAutomaticRetrySecondsV542' in app
    assert 'function invalidateIncompleteTodayFabricationV542' in app
    assert 'todayDateActivationV542' in app
    assert 'refreshIncompleteV542' in app
    assert 'refreshIncomplete' in app[app.index('function requestFabricationBatchV522'):app.index('async function hydrateFabricationStatusesV474')]
    assert 'refresh_incomplete = data.get("refreshIncomplete") is True' in server
    assert 'refresh_missing=refresh_incomplete' in server
    assert '"deliveryDate": str(row.get("deliveryDate") or "")' in server

    # Print modal reveals before initialization and empty preview no longer
    # paginates the complete local dataset.
    preview_block = app[app.index('function renderPrintDocumentPreview'):app.index('function printPreviewFitZoom')]
    assert 'preview.noResults === true' in preview_block
    open_print = app[app.index('function openPrintOptions'):app.index('function closePrintOptions')]
    assert open_print.index('els.printOptionsPanel.hidden = false') < open_print.index('renderEmptyPrintSelectionPreview()')
    preview_css = print_css[print_css.rindex('v0.542 Print preview parity'):]
    assert 'padding: .5in !important;' in preview_css
    assert 'text-overflow: clip !important;' in preview_css
    assert 'width: 6.5% !important;' in preview_css

    # Today's Production Count uses a bounded cache and refreshes after an
    # A+W/catalog synchronization instead of holding the first result all day.
    today_report = app[app.index('async function ensureTodayProductionReportV514'):app.index('async function ensureStatisticsProductionReportV514')]
    assert 'statisticsTodayProductionLoadedAtV542' in today_report
    assert 'reportAgeV542 < 60_000' in today_report
    assert 'state.statisticsTodayProductionLoadedAtV542 = Date.now();' in today_report
    assert 'ensureTodayProductionReportV514({ force: true })' in app


def test_v543_superseded_reoptimization_reject_gradient_and_progress_refresh_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    runner = (ROOT / "automation/sql_delivery_export/Run-DeliveryListSqlAutomation.ps1").read_text(encoding="utf-8-sig")
    scan = (ROOT / "static/css/scan.css").read_text(encoding="utf-8")
    ...
    assert 'v0.543-current-scanner-normal-production-divergence-1' in store
    assert 'def _superseded_cutting_evidence_map' in store
    assert 'replacementHasProductionEvidence' in store
    assert '"batch": str(cutting.get("batch") or "").strip()' in store
    assert '"optimization": int(cutting.get("optimization") or 0)' in store
    assert 'superseded-review-production-pills-v544' in app
    assert 'No production evidence' in app

    assert 'v0.543-same-day-normal-production-divergence-1' in runner
    assert 'COALESCE(NULLIF(seqopt.SequenceOptimization,0),NULLIF(ji.OPTIMIZATION,0))' in runner
    assert 'current PROD_OPTI_SEQUENCE membership is authoritative' in runner

    final_reject = scan[scan.rindex('v0.543 Internal Reject row treatment'):]
    assert 'box-shadow: none !important;' in final_reject
    assert 'var(--attention-internal-reject) 25%' in final_reject
    assert 'inset 3px 0 0' not in final_reject
    assert 'inset -3px 0 0' not in final_reject

    assert 'data-refresh-progress-v543' in app
    assert 'function refreshOrderItemProgressV543(button)' in app
    assert 'fetchOrderDetailCoreV507(order, true)' in app
    assert 'requestFabricationBatchV522(' in app
    assert 'Progress refreshed:' in app
    assert 'data-check-fab-v522' not in app
    assert '<span>Check Fab</span>' not in app
    assert '.production-progress-refresh-v543' in shared
    assert 'production-progress-refresh-spin-v543' in shared


def test_v544_booked_cutting_and_superseded_evidence_layout_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    admin = (ROOT / "static/css/admin.css").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    ...
    # Backend current optimization state owns Cutting completion.
    assert 'current_is_booked = optimization_status in AW_OPTI_STATUS_BOOKED_CODES' in store
    assert 'cannot make a newer Optimized or Released optimization appear cut' in store
    cutting_state = store[store.index('def aw_cutting_state'):store.index('def aw_cutting_sync_plan')]
    assert 'optimization_plate_cut' not in cutting_state
    assert 'prod_jobitem_cut_quantity' not in cutting_state
    assert 'remembered_cutting_completion' not in cutting_state
    assert 'elif optimization_status == AW_OPTI_STATUS_RELEASED:' in cutting_state
    assert 'elif optimization_status == AW_OPTI_STATUS_OPTIMIZED' in cutting_state

    # Browser history/presentation follows the same lifecycle instead of using
    # plate/cut-quantity fields to promote Optimized/Released to Cut.
    presentation = app[app.index('function cuttingGenerationPresentationV500'):app.index('function cuttingLabelProcessRowsV504')]
    assert 'const booked = statusLabel === "booked" || statusCode === 460 || statusCode === 500;' in presentation
    assert 'const released = statusLabel === "released" || statusCode === 200;' in presentation
    assert 'generation.cutCompletedAt' not in presentation
    assert 'generation.cutQuantity' not in presentation
    assert 'optimizationPlateCut' not in presentation

    # Superseded item evidence is card-based and wraps; the old eight-column
    # mini-tables no longer own the form width.
    assert 'superseded-review-item-list-v544' in app
    assert 'superseded-review-production-pills-v544' in app
    superseded = app[app.index('function supersededReviewItemRows'):app.index('function supersededOrderReviewCardHtml')]
    assert '<table>' not in superseded
    assert '<th>Batch</th>' not in superseded
    final_admin = admin[admin.rindex('v0.544 Superseded review evidence cards'):]
    assert 'overflow: hidden;' in final_admin
    assert 'grid-template-columns: minmax(82px, .7fr) minmax(96px, .85fr) minmax(130px, 1.45fr);' in final_admin
    assert '@media (max-width: 1150px)' in final_admin
    assert '@media (max-width: 680px)' in final_admin



def test_v545_order_detail_booked_only_merge_and_timestamp_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    server = (ROOT / "server.py").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    contract = (ROOT / "database/contract.py").read_text(encoding="utf-8")
    ...
    merge = app[app.index('function mergeOrderProductionDetailV507'):app.index('function cachedOrderDetailPayloadV507')]
    assert 'state: "cut"' not in merge
    assert 'inferredFromFabrication' not in merge
    assert 'downstreamFabricationObserved: true' in merge
    assert 'Only Booked completes Cutting' in merge

    timestamps = app[app.index('function cuttingProgressTimestampV511'):app.index('function fabricationProgressTimestampV511')]
    assert 'if (stateName === "cut") return cutting.cutCompletedAt || cutting.optimizationLastChangedAt || "";' in timestamps
    assert 'stateName === "released"' not in timestamps
    assert 'stateName === "optimized"' not in timestamps
    assert 'optimizationPlateLastChangedAt' not in timestamps
    assert 'batchLastChangedAt' not in timestamps

    status_batch = server[server.index('if parsed.path == "/api/production-files/status-batch"'):server.index('if parsed.path == "/api/production-files/open"')]
    assert '"state": "cut"' not in status_batch
    assert '"inferredFromFabrication": True' not in status_batch
    assert '"downstreamFabricationObserved": True' in status_batch
    assert 'current optimization itself must be Booked' in status_batch
    assert 'Cut confirmed by downstream' not in app
    assert 'A+W Cutting completion still requires Booked' in app


def test_v0546_inventory_item_id_lookup_manager_contracts() -> None:
    app = (ROOT / "static/js/app.js").read_text(encoding="utf-8")
    store = (ROOT / "backend/store.py").read_text(encoding="utf-8")
    migrations = (ROOT / "database/migrations.py").read_text(encoding="utf-8")

    assert 'INVENTORY_ITEM_MAPPING_DEFAULTS_V546' in store
    assert '"G38SATINCLR"' in store
    assert '"glassLabel": "3/8 Acid Etch"' in store
    assert '"18CDSWG"' in store
    assert '"glassLabel": "1/8 Clear"' in store
    assert 'def ensure_inventory_item_mapping_defaults' in store
    assert 'def upsert_inventory_item_mapping' in store
    assert '"inventoryItemMappings"' in store
    assert 'if lookup_type == "inventory_item_id"' in store
    assert 'if clean_type == "inventory_item_id"' in store

    assert 'insertTab("lookup:inventory_item_id", "Item IDs"' in app
    assert '"inventoryItemMappings"' in app
    assert 'type: "inventory_item_id"' in app
    assert 'valuePlaceholder: "G38SATINCLR"' in app
    assert 'labelPlaceholder: "3/8 Acid Etch"' in app
    assert 'request.originalItemId = originalValue' in app
    assert 'Remove Item ID' in app

    # v0.546 is a data/default correction and editor addition, not a schema
    # migration. Historical schema-20 seed/checksum content stays immutable.
    assert "('G38SATINCLR', '3/8 Clear Satin'" in migrations
    assert "('18CDSWG', '1/8 Clear DS B-Grade Glass'" in migrations


def test_v0547_inventory_generated_xlsx_uses_current_item_id_mappings() -> None:
    store = (ROOT / "backend" / "store.py").read_text(encoding="utf-8")
    contract = (ROOT / "database" / "contract.py").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    ...
    assert 'aria-label="Application version 0.548"' in html
    assert '<strong>0.548</strong>' in html
    # app.js itself did not change in v0.547, so its browser cache key stays put.
    assert 'static/js/app.js?v=20260918-v0.546' in html
    assert 'Current maintained release: **v0.548**' in readme
    assert changelog.startswith('## v0.548 - Inventory Action Accessibility and Control Alignment')
    assert 'The unrelated standalone `Inventory.xlsx` is not part' in readme

    export_start = store.index('def export_inventory_xlsx')
    export = store[export_start:store.index('def export_csv', export_start)]
    assert 'export_mappings = []' in export
    assert 'def export_item_id(glass_type: Any, stored_item_id: Any = "") -> str:' in export
    assert 'mapping.get("matchTerms")' in export
    assert 'export_item_id(item["glassType"], item["itemId"])' in export
    assert 'export_item_id(system_item.get("glassType", ""), system_item.get("itemId", ""))' in export
    assert 'export_item_id(physical.get("glassType", ""), physical.get("itemId", ""))' in export
    assert 'export_totals: dict[tuple[str, str], dict[str, Any]] = {}' in export
    assert 'inventory_expected_items' not in export or 'UPDATE inventory_expected_items' not in export
    assert 'UPDATE inventory_scans' not in export


def test_v0548_inventory_actions_start_controls_and_manual_close_contracts() -> None:
    inventory = (ROOT / "static/css/inventory.css").read_text(encoding="utf-8")
    html = (ROOT / "index.html").read_text(encoding="utf-8")
    contract = (ROOT / "database/contract.py").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    changelog = (ROOT / "README_CHANGELOG.md").read_text(encoding="utf-8")
    ...
    final_inventory = inventory[inventory.rindex('v0.548 Inventory session actions'):]
    assert '.inventory-session-header {' in final_inventory
    assert 'position: sticky;' in final_inventory
    assert 'z-index: 8;' in final_inventory
    assert 'pointer-events: auto;' in final_inventory
    assert '.inventory-workspace > .inventory-tabs {' in final_inventory
    assert 'position: relative;' in final_inventory
    assert 'grid-template-columns: repeat(3, minmax(0, 1fr));' in final_inventory
    assert 'height: 44px;' in final_inventory
    assert '--gui-close-position: absolute;' in final_inventory
    assert '--gui-close-top: 14px;' in final_inventory
    assert '--gui-close-right: 14px;' in final_inventory
    assert '@media (max-width: 720px)' in final_inventory
