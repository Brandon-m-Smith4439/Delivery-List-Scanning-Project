...
def test_header_contains_only_title_status_and_route_pulse() -> None:
    ...
    mode_summary = panel.select_one(".bay-scanner-mode-summary-v147")
...
def test_route_pulse_removes_legacy_connector_and_white_pill() -> None:
    ...
    assert ".bay-route-metrics-v147 .bay-panel-route-lane::before" in css
    assert ".bay-route-metrics-v147 .bay-panel-route-lane::after" in css
    assert ".bay-route-metrics-v147 .bay-panel-route-lane > span::before" in css
...
def test_destination_control_is_mode_specific() -> None:
    _, panel = scanner_panel()
    target = panel.select_one(".bay-command-target-v147")
    assert target is not None
    assert target.select_one("#bayScanBayInput") is not None
    css = read("bay-scanner-v147.css")
    assert ".bay-scanner-panel-v147:has(#bayScanRemoveMode:checked) .bay-command-target-v147" in css
    assert ".bay-scanner-panel-v147:has(#bayScanModeToggle:checked) .bay-command-target-v147" in css
    assert re.search(
        r":has\(#bayScanRemoveMode:checked\) \.bay-command-target-v147\s*\{[^}]*display:\s*none\s*!important",
        css,
        flags=re.S,
    )
    assert re.search(
        r":has\(#bayScanModeToggle:checked\) \.bay-command-target-v147\s*\{[^}]*display:\s*grid\s*!important",
        css,
        flags=re.S,
    )
...
def test_only_scanner_slot_is_sticky_and_moves_to_top() -> None:
    ...
    sticky_slot = panel.find_parent(class_="bay-scanner-sticky-slot-v147")
    assert action_toolbar is not None and sticky_slot is not None
    assert action_toolbar not in sticky_slot.descendants
    css = read("bay-scanner-v147.css")
    assert re.search(
        r"\.bay-scanner-sticky-slot-v147\s*\{[^}]*position:\s*sticky\s*!important[^}]*top:\s*8px\s*!important",
        css,
        flags=re.S,
    )
    assert re.search(
        r"\.bay-action-buttons-v147\s*\{[^}]*position:\s*static\s*!important[^}]*top:\s*auto\s*!important",
        css,
        flags=re.S,
    )
