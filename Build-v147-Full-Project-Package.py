#!/usr/bin/env python3
"""Build a clean full-project v147 ZIP from the installed local project folder."""
...
def excluded(relative: Path) -> bool:
    ...
    if any(name.endswith(suffix) for suffix in EXCLUDED_SUFFIXES):
        return True
    if name.startswith("delivery_list_scanner_v147_") and name.endswith(".zip"):
        return True
    return False
...
def main() -> int:
    ...
    required = (
        ...
        project_root / "bay-scanner-v147.css",
    )
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise RuntimeError("v147 is not fully installed: " + ", ".join(missing))

    if "20260728-v147" not in (project_root / "index.html").read_text(encoding="utf-8"):
        raise RuntimeError("index.html is not using the v147 cache key")

    output = Path(args.output).expanduser().resolve() if args.output else (
        project_root.parent / "Delivery_List_Scanner_v147_Full_Project.zip"
    )

    with tempfile.TemporaryDirectory(prefix="delivery-scanner-v147-") as temp_name:
        staging = Path(temp_name) / "Delivery_List_Scanner_v147"
        ...
        (staging / "FULL_PROJECT_PACKAGE_MANIFEST.json").write_text(
            json.dumps({"release": 147, "files": manifest}, indent=2) + "\n",
            encoding="utf-8",
        )
