# Delivery List Scanner v063 Test Report

Date: 2026-07-15

## Result

**47 automated tests passed. No tests were skipped.**
...
- v060-created SQLite database upgrade and v063 startup.
...
- v063 cache references are present.
...
## v062 diagnostic result and v063 production crash fix

v062's durable diagnostics exposed the exact production exception: `seed_demo_data()` was synchronizing an old `data/sample-delivery-list.json` into a non-empty production database. One deterministic sample-derived ID had previously moved to another stage, so rebuilding the original list attempted to reuse the globally unique ID and SQLite stopped startup.

v063 protects this in two layers. Demo data seeds only into an empty database, and the shared line-item insertion workflow derives a stable collision-safe ID when an older stage move owns the desired ID in another list. A regression test recreates the exact condition, and a real local HTTP server started successfully against that collision database.
...
See `CODE_REFERENCE_v063.md` for the function-by-function map and `TESTING_v063.md` for release and floor-validation steps.
