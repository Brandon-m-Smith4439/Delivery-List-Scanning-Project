...
Current maintained release: **v169**. SQLite remains the active/default backend.

v169 adds a Glass Type section to the Manual Delivery List Edit filter drawer. The available buttons are generated from the glass/product types that actually exist in the selected delivery-list stage, include current piece quantities, support selecting multiple glass types, and combine with the existing progress, route, location, attention, and text-search filters.

## Install v169 over v168

1. Close the Delivery List Scanner server.
2. Extract `Delivery_List_Scanner_v169_Manual_Edit_Glass_Type_Filters_Changed_Files.zip` into the current project folder and replace the included files.
...
- Preserves the v168 exact-row Save repair and every existing manual-edit workflow.
...
- Preserves the v167 explicit route override and New Order workspace layout.
