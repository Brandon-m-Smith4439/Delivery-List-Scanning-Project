...
- Normalized **Location**, **Count Type**, and **Start Inventory** into one three-column control row with matching 44px control heights at desktop/tablet widths; mobile keeps the maintained one-column touch layout.
- Corrected the Manual Inventory Entry X by using the shared GUI-close positioning variables, placing it at the top-right without creating a duplicate close-control implementation.
- Advanced `APPLICATION_VERSION` exactly one step from **547** to **548** and advanced the changed Inventory CSS cache key. SQLite remains schema **21** with no migration.
- Validation passes JavaScript syntax and Python source parsing; focused Inventory regressions pass **9/9**, static/UI contracts pass **246/246**, and the complete maintained suite passes **368/368**. A local Chromium file probe did not complete, so exact floor-browser hit testing remains a controlled deployment check.

## v0.547 - Inventory Export Item ID Sync Correction
- Corrected the v0.546 spreadsheet interpretation: the relevant Inventory workbook is the XLSX generated by the web app during or after a physical Inventory session; the unrelated standalone `Inventory.xlsx` is not part of this release/runtime workflow.
- Updated `export_inventory_xlsx` so Item IDs are resolved from the current active Lookup Manager mapping at export time across **Summary**, **Physical Scans**, **System Snapshot**, and **Reconciliation**.
- Export-time matching uses the same canonical glass labels, descriptions, and aliases as Inventory Smart Fill, so pre-existing sessions with blank/legacy Item IDs can still export the current maintained `G38SATINCLR` and `18CDSWG` mappings.
- Rebuilt the export Summary grouping on the current resolved Item ID so a mapping correction does not create duplicate glass totals solely because the frozen session retained an older/blank Item ID.
- Preserved the immutable inventory audit contract: `inventory_expected_items` and `inventory_scans` are not rewritten when mappings change; the correction is presentation-only in generated XLSX output.
- Advanced `APPLICATION_VERSION` exactly one step from **546** to **547**. No CSS/JavaScript runtime asset changed, so existing browser cache keys remain unchanged. SQLite schema remains **21** with no migration.
- Validation passes JavaScript syntax; all **37** Python sources parse without bytecode churn and runtime imports report application **547** / schema **21**; focused Item ID/default/export regressions pass **3/3**; static/UI contracts pass **245/245**; and the complete maintained suite passes **367/367**.
- A generated Inventory XLSX was validated across Summary, Physical Scans, System Snapshot, and Reconciliation: current `G38SATINCLR` / `18CDSWG` mappings are present, a simulated stale `LEGACY18` ID is absent, and the maintained filters/freeze panes/column widths remain intact.
- An isolated production-scale SQLite copy upgraded from schema ledger **11 → 21** after a SHA-256-verified backup while preserving **338 delivery lists / 22,754 line items / 2,080 scan events / 3 reject events / 663 imports / 64 bay assignments**; `integrity_check` is **ok** and `foreign_key_check` reports **0** violations.

## v0.546 - Inventory Item ID Management
- Added **Lookup Manager → Item IDs** using the existing `inventory_item_mappings` store so administrators can add, edit, search, and remove the Item ID associated with each physical inventory glass type.
- Corrected the two requested maintained mappings without changing schema: **3/8 Acid Etch → G38SATINCLR** and **1/8 Clear → 18CDSWG**. Legacy Satin/DS B-Grade wording remains in match terms so older A+W text still resolves to the same Item IDs.
- Added an idempotent startup repair that updates only untouched legacy seed labels or inserts a missing requested default; inactive mappings stay inactive and administrator-renamed mappings are never overwritten.
- Reused the existing generic Lookup Manager API and audit stream for Item ID create/update/remove operations; duplicate active glass labels and conflicting Item IDs are rejected instead of creating ambiguous Smart Fill behavior.
- Inventory Smart Fill, frozen snapshots, reconciliation, and the maintained Inventory XLSX exporter continue to read the same mapping table, so saved Item ID changes flow through to generated Excel exports automatically.
- Updated the supplied companion Inventory workbook as `Inventory_v0.546.xlsx` with a filtered/frozen **Glass Item IDs** worksheet containing all 15 maintained mappings. The original Inventory worksheet, external-link parts, formulas, styles, print settings, and calculation-chain payloads are preserved byte-for-byte.
- Advanced `APPLICATION_VERSION` exactly one step from **545** to **546**, advanced the changed application JavaScript cache key, and kept SQLite schema at **21** with no migration.
- Validation passes JavaScript syntax; all **37** Python sources parse without bytecode churn and changed modules import as application **546** / schema **21**; focused Inventory Item ID regressions pass **2/2**; static/UI contracts pass **244/244**; and the complete maintained suite passes **365/365**.
- An isolated production-scale SQLite copy upgraded through the existing numbered migrations from schema ledger **11 → 21** after a verified backup while preserving **338 delivery lists / 22,754 line items / 2,080 scan events / 3 reject events / 663 imports / 64 bay assignments**. Integrity is **ok**, foreign-key violations are **0**, and the Item ID mapping table retains **15** active mappings including the two corrected defaults.
- An isolated fresh v0.546 server returned healthy `/api/health`, initialized schema **21**, exposed both corrected default mappings, passed integrity/foreign-key checks, and was stopped afterward.

## v0.545 - Order Details Booked-Only Cutting State
- Removed the remaining Order Details production-file merge rule that promoted completed Denver/WaterJet fabrication into A+W Cutting `Cut`. Downstream fabrication is now diagnostic context only; the current optimization must itself be Booked.
- Removed the same promotion from the focused `/api/production-files/status-batch` refresh path, so using the Order Details Progress refresh icon cannot overwrite an Optimized/Released Cutting state.
- Tightened the Cutting checkpoint timestamp contract: Optimized, Released, Batch-active, unknown, and other incomplete states render no date/time. Only Cut/Booked renders a completion timestamp, preferring the synchronized cutting booking time and then the optimization status-change time.
- Advanced `APPLICATION_VERSION` exactly one step from **544** to **545**, advanced the changed application JavaScript cache key, and kept SQLite schema at **21** with no migration.
- Validation passes JavaScript syntax and the maintained Scan reference contract; all **37** Python sources parse without bytecode churn and changed modules import as application **545** / schema **21**; focused Order Details/Booked-only regressions pass **4/4**; static/UI contracts pass **243/243**; and the complete maintained suite passes **362/362**.
- An isolated fresh v0.545 server returned healthy `/api/health`, initialized schema ledger **21**, returned `integrity_check: ok` with **0** foreign-key violations, and was stopped afterward.

## v0.544 - Booked-Only Cutting Completion and Superseded Review Cleanup
- Corrected A+W Cutting semantics so the **current optimization lifecycle** is authoritative: Optimized remains incomplete, Released means Cutting is in progress, and only Booked marks Cutting complete.
- Removed completion promotion from plate `CUT`/`STOCKBOOKED`, `MENGE_CUT`, retained cutting-booking timestamps, and remembered older generations. Those fields remain retained as diagnostic/history evidence.
- Prevented a previously Booked optimization in the same KEYINDEX from making a newer reoptimized Optimized/Released generation appear cut. Reject cutoffs still reset the lifecycle as before.
- Updated Cutting history presentation to follow the same Booked-only rule so historical/current cards do not contradict the main progress state.
- Reworked Superseded Order item evidence from wide eight-column mini-tables into compact item cards with grouped Glass/Job, Size, A+W status, Batch, Optimization, and Cutting evidence. The layout wraps instead of requiring horizontal scrolling.
- Advanced `APPLICATION_VERSION` exactly one step from **543** to **544**, advanced cache keys for changed `admin.css` and `app.js`, and kept SQLite schema at **21** with no migration.
- Validation passes JavaScript syntax and the Scan reference contract; all **37** Python sources parse/import cleanly as application **544** / schema **21**; focused Cutting lifecycle regressions pass **9/9**; static/UI contracts pass **242/242**; and the complete maintained suite passes **361/361**.
- An isolated copy of the bundled SQLite database upgraded through the existing numbered migrations from schema ledger **11 -> 21** after a verified byte-for-byte backup while preserving **338 delivery lists / 22,754 line items / 2,080 scan events / 3 reject events / 663 imports / 64 bay assignments**. `integrity_check` returned **ok** and `foreign_key_check` returned **0** violations.
- An isolated fresh v0.544 server returned healthy `/api/health`, reached schema **21**, passed integrity/foreign-key checks, and was stopped afterward. Headless Chromium did not complete a reliable local visual render, so final responsive/TC22 Superseded-form appearance remains a controlled deployment check.

## v0.543 - Superseded Production Duplicates, Current Optimization, and Progress Refresh
- Extended Superseded detection beyond Remake/header-identity cases: exact same-day normal orders with the same Job, Customer, and exact item set now enter the existing review queue when one copy has production evidence and the other has none. The unproduced copy is suggested for removal; nothing is auto-deleted.
- Added the same normal/normal production-divergence rule to the A+W import-time detector so new duplicate pairs can be queued during synchronization instead of requiring only a later manual refresh.
- Enriched Superseded item evidence from one batched `aw_cutting_generations` read. The GUI now shows current Batch, Optimization, and Cutting state and uses an em dash when evidence is genuinely unavailable instead of presenting misleading zero values.
- Corrected current A+W optimization selection so `PROD_OPTI_SEQUENCE` membership takes precedence over a stale `PROD_JOBITEM.OPTIMIZATION`. The job-item field remains a fallback, and older Cutting generations remain retained for history/reject-remake lifecycle evidence.
- Added regression coverage for a two-item normal/normal duplicate where only one order has booked Cutting evidence, plus same-batch reoptimization from an older optimization to the newly assigned one.
- Removed the v0.542 Internal Reject left/right orange rails and strengthened only the existing row gradient for a cleaner continuous reject treatment.
- Replaced Order Details `Check Fab` with a compact Progress refresh icon. One refresh now forces the focused Order Details core/stage data and that item's Cutting/fabrication status together, then repaints the open Order Details and Scan progress.
- Advanced `APPLICATION_VERSION` exactly one step from **542** to **543**, advanced cache keys for changed `scan.css`, `shared-ui.css`, and `app.js`, and kept SQLite schema at **21** with no migration.
- Validation passes JavaScript syntax and the Scan reference contract; all **37** Python sources parse/import cleanly; focused Superseded/Reoptimization/Cutting regressions pass **10/10**; fabrication-memory/reference regressions pass **10/10**; static/UI contracts pass **241/241**; and the complete maintained suite passes **359/359**.
- An isolated bundled-database copy upgraded through the existing numbered migrations from schema ledger **11 -> 21** after a verified backup while preserving **338 delivery lists / 22,754 line items / 2,080 scan events / 3 reject events / 663 imports / 64 bay assignments**. `integrity_check` returned **ok** and `foreign_key_check` returned **0** violations.
- An isolated fresh v0.543 server returned healthy `/api/health`, reached schema **21**, passed integrity/foreign-key checks, and was stopped. Native PowerShell is unavailable in this Linux environment, and the older bundled database does not contain the pictured 238767/238724 floor pair; the equivalent two-item normal/normal production-divergence scenario is protected by regression coverage and the exact floor pair remains a controlled deployment check.

## v0.542 - Zoom-Safe Scan Layout, Live Today FAB, and Print Preview Parity
- Preserved the v0.541 100% desktop look while adding short-viewport desktop ownership for 110%, 125%, 150%, and 175% browser zoom: Scan list/scanner columns no longer collide and the desktop sidebar navigation becomes independently scrollable so every page remains reachable.
- Added a more compact short-height sidebar mode below 680 CSS pixels without changing the normal desktop rail, mobile drawer, or TC22 breakpoint ownership.
- Changed current-day incomplete fabrication from the historical ten-minute retry floor to a bounded one-minute retry, invalidated incomplete browser FAB freshness on a real switch back to today, and added a foreground `refreshIncomplete` status-batch path that asks the existing production-file service to refresh only missing evidence.
- Kept historical/future FAB checks on the existing ten-minute low-I/O cadence and retained completed fabrication as a durable lifecycle milestone.
- Expanded the Mark Reviewed share inside review cards and increased its readable label/icon sizing without increasing the overall card footprint.
- Added subtle orange left/right edge markers to Internal Reject physical item rows while preserving the existing orange incident ribbon and glass/priority styling.
- Made Print/Export reveal its shell before initialization work and prevented the empty preview state from synchronously building/paginating every printable row.
- Corrected Remake preview geometry to use the same `.5in` inner content inset as printed Remake sheets so content stays inside the dashed border, and gave the preview Check column explicit non-ellipsis ownership.
- Added a one-minute freshness window to Today’s Production Count plus immediate invalidation/refetch after an A+W/delivery-catalog synchronization while Statistics is visible, preventing a morning `No plates` snapshot from remaining stale after plate data arrives.
- Advanced `APPLICATION_VERSION` exactly one step from **541** to **542**, advanced cache keys for changed `styles.css`, `scan.css`, `print.css`, and `app.js`, and kept SQLite schema at **21** with no migration.
- JavaScript syntax passes, all maintained Python sources parse/import without bytecode churn, the final Scan reference regression passes, and the complete maintained suite passes **356/356**.
- An isolated copy of the bundled SQLite test database upgraded through the existing numbered migrations from schema ledger **11 -> 21** after creating the migration backup. Row counts remained **338 delivery lists / 22,754 line items / 2,080 scan events / 3 reject events / 663 imports / 64 bay assignments**, `integrity_check` returned **ok**, and `foreign_key_check` returned **0** violations.
- The bundled database predates the current live physical-plate snapshot, so it cannot reproduce the floor's current-day plate contents. The Today Production Count repair is therefore protected through cache/sync contracts; live A+W plate refresh remains a controlled floor validation.
- Browser-level localhost screenshots remain unavailable in this environment because Chromium blocks localhost. The user-supplied 110%-150% screenshots drove the short-height ownership fixes; final 110%, 125%, 150%, 175%, TC22, live-share, and printer visual checks remain controlled deployment checks.

## v0.541 - FAB Date-Switch Reliability, Scan Override, and Faster Bay Map
- Increased Scan progress checkpoints by about 10%, centered them within the Progress column, and unified Denver/WaterJet typography with Cutting and workflow stages.
- Removed the pending-fabrication color override that could force WaterJet/Denver to inherit the previous checkpoint color; incomplete machine cards now retain their Lookup Manager color.
- Widened Mark Reviewed actions so their icon and text remain comfortably inside the review cards.
- Reduced the delayed Checking fabrication reveal from 3 seconds to 1 second.
- Fixed FAB cache eviction so status, Cutting, and freshness markers are discarded together; revisiting an evicted delivery date now performs the required replacement check.
- Allowed current-date fabrication requests to bypass speculative future-date prewarm queues, resumed interrupted current-date warms after rapid date switching, and repainted only visible progress cells as each batch completed.
- Added an explicit Staging fabrication override confirmation. Successful overrides persist Cutting plus assigned-machine completion for the current lifecycle and are written to Action History only after rack/destination gates accept the scan.
- Removed the Bay Map assignment N+1 read by loading active assignments and latest scan facts in one batched query, while preserving the existing bay payload contract.
- Split Bay Map critical layout/bay loading from supplemental summary/history requests and deferred the decorative transfer animation until after usable content paints.
- Added responsive ownership for progress/review controls at effective desktop widths corresponding to 110%, 125%, 150%, and 175% browser zoom.
- Advanced `APPLICATION_VERSION` exactly one step from **540** to **541** and advanced cache keys for changed `scan.css` and `app.js`. SQLite schema remains **21**; no migration was added.
- Validation passes JavaScript syntax, maintained Python parse/import checks, **239/239** static/UI contracts, **17/17** focused fabrication/WaterJet/Bay regressions, and the complete maintained suite at **354/354**.
- An isolated bundled-database copy upgraded from schema ledger **11 -> 21** with all existing delivery-list/line-item/scan/reject/import/bay-assignment row counts preserved, `integrity_check: ok`, and **0** foreign-key violations. Direct batched Bay Map reads measured about **9.3 ms median** for 934 bays / 30 active assignments.
- An isolated v0.541 server reached healthy `/api/health` and was stopped. Chromium is blocked from localhost by environment policy, so requested 110%-175% zoom screenshots and floor-only TC22/live-share/printer behavior remain deployment checks.
