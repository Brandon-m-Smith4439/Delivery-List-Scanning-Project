#!/usr/bin/env python3
"""Build a clean full-project v146 ZIP from the installed local project folder."""
...
def excluded(relative: Path) -> bool:
    ...
    if any(name.endswith(suffix) for suffix in EXCLUDED_SUFFIXES):
        return True
    if name.startswith("delivery_list_scanner_v146_") and name.endswith(".zip"):
        return True
    return False
...
def main() -> int:
    ...
    if missing:
        raise RuntimeError("v146 is not fully installed: " + ", ".join(missing))

    if "20260728-v146" not in (project_root / "index.html").read_text(encoding="utf-8"):
        raise RuntimeError("index.html is not using the v146 cache key")

    output = Path(args.output).expanduser().resolve() if args.output else (
        project_root.parent / "Delivery_List_Scanner_v146_Full_Project.zip"
    )

    with tempfile.TemporaryDirectory(prefix="delivery-scanner-v146-") as temp_name:
        staging = Path(temp_name) / "Delivery_List_Scanner_v146"
        ...
        (staging / "FULL_PROJECT_PACKAGE_MANIFEST.json").write_text(
            json.dumps({"release": 146, "files": manifest}, indent=2) + "\n",
            encoding="utf-8",
        )
