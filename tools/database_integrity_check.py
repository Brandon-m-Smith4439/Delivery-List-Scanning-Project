...
from database_contract import (  # noqa: E402
...
from database_migrations import MIGRATIONS  # noqa: E402
...
def check_database(database_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        for name, query in duplicate_queries.items():
            table = name.split("_code")[0] + "s" if name.endswith("_code") else name.split("_", 2)[0] + "_items"
            try:
