...
if ($release -gt 115) {
    throw "The selected project is v$release. This v115 installer will not modify a newer release."
}
...
$backupRoot = Join-Path $WorkingRoot ("Backups\v115-project-integration-{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
...
# Keep the main application assets in place and replace only the isolated v107-v115 add-on references.
...
$indexText = [regex]::Replace($indexText, '<link rel="stylesheet" href="styles\.css\?v=[^"]+">', '<link rel="stylesheet" href="styles.css?v=20260723-v115">', 1)
$indexText = [regex]::Replace($indexText, '<script src="app\.js\?v=[^"]+"></script>', '<script src="app.js?v=20260723-v115"></script>', 1)
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<link rel="stylesheet" href="styles\.css\?v=20260723-v115">' `
    -Addition ("`r`n    " + '<link rel="stylesheet" href="delivery-automation-ui.css?v=20260723-v115">' + "`r`n    " + '<link rel="stylesheet" href="notification-center-ui.css?v=20260723-v115">') `
    -Label "main stylesheet"
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<script src="app\.js\?v=20260723-v115"></script>' `
    -Addition ("`r`n  " + '<script src="delivery-automation-ui.js?v=20260723-v115"></script>' + "`r`n  " + '<script src="notification-center-ui.js?v=20260723-v115"></script>') `
...
if (-not $appText.Contains("DLS_AUTOMATION_LIST_REFRESH_BRIDGE_V115")) {
    $appStateAnchor = @'
  lastGlassFilterSignature: "",
};
'@
...
// DLS_AUTOMATION_LIST_REFRESH_BRIDGE_V115
...
$readmeText = [regex]::Replace($readmeText, 'Current maintained release:\s*\*\*v\d+\*\*\.', 'Current maintained release: **v115**.', 1)
if (-not $readmeText.Contains('v115 adds non-disruptive live delivery-list synchronization')) {
    $marker = "Current maintained release: **v115**. SQLite remains the active/default backend.`r`n"
    if (-not $readmeText.Contains($marker)) {
        $marker = "Current maintained release: **v115**. SQLite remains the active/default backend.`n"
    }
    $addition = @'
...
if (-not $changelogText.Contains('## v115')) {
    $entry = @'
...
Write-Host "v115 project integration applied successfully." -ForegroundColor Green
