...
if ($release -gt 120) {
    throw "The selected project is v$release. This v120 installer will not modify a newer release."
}
...
$backupRoot = Join-Path $WorkingRoot ("Backups\v120-project-integration-{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
...
# v120 adds a numbered, backup-protected SQLite migration for per-user line-update
# notices. The migration is additive and never edits existing scan or audit history.
$databaseContractText = [regex]::Replace($databaseContractText, 'APPLICATION_VERSION = "[^"]+"', 'APPLICATION_VERSION = "120"', 1)
...
'@
    ...
    $databaseMigrationsText = Replace-RequiredText -Text $databaseMigrationsText -Old $migrationTupleAnchor -New $migrationTupleBlock -Label "v120 migration tuple"
}

# Keep the main application assets in place and replace only the isolated v107-v120 add-on references.
...
$indexText = [regex]::Replace($indexText, '<link rel="stylesheet" href="styles\.css\?v=[^"]+">', '<link rel="stylesheet" href="styles.css?v=20260723-v120">', 1)
$indexText = [regex]::Replace($indexText, '<script src="app\.js\?v=[^"]+"></script>', '<script src="app.js?v=20260723-v120"></script>', 1)
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<link rel="stylesheet" href="styles\.css\?v=20260723-v120">' `
    -Addition ("`r`n    " + '<link rel="stylesheet" href="delivery-automation-ui.css?v=20260723-v120">' + "`r`n    " + '<link rel="stylesheet" href="notification-center-ui.css?v=20260723-v120">') `
    -Label "main stylesheet"
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<script src="app\.js\?v=20260723-v120"></script>' `
    -Addition ("`r`n  " + '<script src="delivery-automation-ui.js?v=20260723-v120"></script>' + "`r`n  " + '<script src="notification-center-ui.js?v=20260723-v120"></script>') `
...
if ([regex]::IsMatch($indexText, $deliveryManagementHeadingPattern)) {
    ...
    throw "Could not find the Delivery List Management heading for the v120 control-center integration."
}
...
# v120 bridge. Merely renaming the marker would preserve the stale behavior.
$automationBridgePattern = '(?s)\n// DLS_AUTOMATION_LIST_REFRESH_BRIDGE_V(?:115|116|117|118|119|120)\n.*?\n\}\);\n'
...
// DLS_AUTOMATION_LIST_REFRESH_BRIDGE_V120
...
elseif (-not $serverText.Contains('latest_import = DELIVERY_AUTOMATION.get_latest_import_result()')) {
    throw "Could not find the Admin summary route for the v120 latest-import integration."
}
...
$readmeText = [regex]::Replace($readmeText, 'Current maintained release:\s*\*\*v\d+\*\*\.', 'Current maintained release: **v120**.', 1)
if (-not $readmeText.Contains('v120 separates delivery-list automation notices from Rush alerts')) {
    $marker = "Current maintained release: **v120**. SQLite remains the active/default backend.`r`n"
    if (-not $readmeText.Contains($marker)) {
        $marker = "Current maintained release: **v120**. SQLite remains the active/default backend.`n"
    }
    $addition = @'
...
if (-not $changelogText.Contains('## v120')) {
    $entry = @'
...
Write-Host "v120 project integration applied successfully." -ForegroundColor Green
