...
if ($release -gt 112) {
    throw "The selected project is v$release. This v112 installer will not modify a newer release."
}
...
$backupRoot = Join-Path $WorkingRoot ("Backups\v112-project-integration-{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
...
# Keep the main application assets in place and replace only the isolated v107-v112 add-on references.
...
$indexText = [regex]::Replace($indexText, '<link rel="stylesheet" href="styles\.css\?v=[^"]+">', '<link rel="stylesheet" href="styles.css?v=20260722-v112">', 1)
$indexText = [regex]::Replace($indexText, '<script src="app\.js\?v=[^"]+"></script>', '<script src="app.js?v=20260722-v112"></script>', 1)
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<link rel="stylesheet" href="styles\.css\?v=20260722-v112">' `
    -Addition ("`r`n    " + '<link rel="stylesheet" href="delivery-automation-ui.css?v=20260722-v112">' + "`r`n    " + '<link rel="stylesheet" href="notification-center-ui.css?v=20260722-v112">') `
    -Label "main stylesheet"
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<script src="app\.js\?v=20260722-v112"></script>' `
    -Addition ("`r`n  " + '<script src="delivery-automation-ui.js?v=20260722-v112"></script>' + "`r`n  " + '<script src="notification-center-ui.js?v=20260722-v112"></script>') `
...
$readmeText = [regex]::Replace($readmeText, 'Current maintained release:\s*\*\*v\d+\*\*\.', 'Current maintained release: **v112**.', 1)
if (-not $readmeText.Contains('v112 fixes false failures when an incremental check has no workbooks to import')) {
    $marker = "Current maintained release: **v112**. SQLite remains the active/default backend.`r`n"
    if (-not $readmeText.Contains($marker)) {
        $marker = "Current maintained release: **v112**. SQLite remains the active/default backend.`n"
    }
    $addition = @'
...
if (-not $changelogText.Contains('## v112')) {
    $entry = @'
...
Write-Host "v112 project integration applied successfully." -ForegroundColor Green
