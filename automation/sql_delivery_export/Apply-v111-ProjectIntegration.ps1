...
if ($release -gt 111) {
    throw "The selected project is v$release. This v111 installer will not modify a newer release."
}
...
$backupRoot = Join-Path $WorkingRoot ("Backups\v111-project-integration-{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
...
# Keep the main application assets in place and replace only the isolated v107-v111 add-on references.
...
$indexText = [regex]::Replace($indexText, '<link rel="stylesheet" href="styles\.css\?v=[^"]+">', '<link rel="stylesheet" href="styles.css?v=20260722-v111">', 1)
$indexText = [regex]::Replace($indexText, '<script src="app\.js\?v=[^"]+"></script>', '<script src="app.js?v=20260722-v111"></script>', 1)
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<link rel="stylesheet" href="styles\.css\?v=20260722-v111">' `
    -Addition ("`r`n    " + '<link rel="stylesheet" href="delivery-automation-ui.css?v=20260722-v111">' + "`r`n    " + '<link rel="stylesheet" href="notification-center-ui.css?v=20260722-v111">') `
    -Label "main stylesheet"
$indexText = Add-AssetAfterMatch -Text $indexText `
    -Pattern '<script src="app\.js\?v=20260722-v111"></script>' `
    -Addition ("`r`n  " + '<script src="delivery-automation-ui.js?v=20260722-v111"></script>' + "`r`n  " + '<script src="notification-center-ui.js?v=20260722-v111"></script>') `
...
$readmeText = [regex]::Replace($readmeText, 'Current maintained release:\s*\*\*v\d+\*\*\.', 'Current maintained release: **v111**.', 1)
if (-not $readmeText.Contains('v111 fixes the completed-import log stall and reduces live-status write overhead')) {
    $marker = "Current maintained release: **v111**. SQLite remains the active/default backend.`r`n"
    if (-not $readmeText.Contains($marker)) {
        $marker = "Current maintained release: **v111**. SQLite remains the active/default backend.`n"
    }
    $addition = @'
...
if (-not $changelogText.Contains('## v111')) {
    $entry = @'
...
Write-Host "v111 project integration applied successfully." -ForegroundColor Green
